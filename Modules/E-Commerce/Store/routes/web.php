...
Route::get('/notifications', function () {
    ...
    $notifications = $user && $clientId
        ? \Modules\Ecommerce\Models\CustomerNotification::forClient($clientId)
            ->visibleTo((int) $user->id)
            ->latest()
            ->paginate(25)
        : collect();

    return view('ecommerce::notifications', compact('notifications'));
})->name('notifications');

Route::middleware([\Modules\Ecommerce\Http\Middleware\RequireEcommerceAuth::class])
...
Route::middleware([\Modules\Ecommerce\Http\Middleware\RequireEcommerceAuth::class])->group(function () {
    ...
    Route::put('/account/password', [\Modules\Ecommerce\Http\Controllers\AccountController::class, 'updatePassword'])->name('account.password.update');
