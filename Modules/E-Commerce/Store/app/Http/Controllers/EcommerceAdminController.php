...
class EcommerceAdminController extends Controller
{
    ...
    public function updateOrderStatus(Request $request, string $id): \Illuminate\Http\JsonResponse
    {
        ...
        CustomerNotification::create([
            ...
            'type' => 'order_status',
            'title' => 'Order '.$labels[$data['status']],
            'body' => 'Your order '.($order->tracking_number ?: '#'.$order->id).' is now '.$labels[$data['status']].'.',
            'link' => route('ecommerce.account.orders.show', ['store' => $company->ecommerce_slug, 'id' => $order->id]),
            'icon' => $data['status'] === 'delivered' ? 'ph-check-circle' : 'ph-truck',
            'icon_color' => $data['status'] === 'cancelled' ? 'red' : 'primary',
            'is_read' => false,
        ]);
    ...
    public function customerNotifications()
    {
        ...
        return view('ecommerce::admin.customer-notifications', [
            'notifications' => CustomerNotification::forClient((int) $company->id)->latest()->paginate(25),
        ]);
    }
    ...
    public function customerNotificationsStore(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('ecommerce.admin.customer-notifications')->with('success', 'Customer notification published.');
    }

    public function customerNotificationsDelete(int $id): RedirectResponse
    {
        $company = $this->company();
        CustomerNotification::forClient((int) $company->id)->whereKey($id)->firstOrFail()->delete();

        return back()->with('success', 'Customer notification removed.');
    }
