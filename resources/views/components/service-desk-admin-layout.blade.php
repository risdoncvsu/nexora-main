    ...
    <div class="flex min-h-screen flex-col">
        ...
        <main class="relative flex-1 overflow-hidden p-6">
            ...
            <section class="relative z-10 grid gap-6 lg:grid-cols-[22rem_1fr]">
                <aside class="rounded-[1.875rem] bg-white p-8 text-slate-950"><nav class="space-y-6 text-xl">@foreach ($links as $key => $link)<a href="{{ $link['route'] }}" class="block {{ $section === $key ? 'font-extrabold text-[#132B52]' : 'font-medium hover:text-[#346DCB]' }}">{{ $link['label'] }}</a>@endforeach</nav></aside>
                <div class="space-y-6">
